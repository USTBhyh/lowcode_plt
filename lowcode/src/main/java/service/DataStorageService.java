...
public class DataStorageService {
    ...
    public void storeFormData(String subUrl) throws Exception{
        //存储form相关信息
    ...
    public void storeData(String jsonString, String subUrl) throws Exception{
        ...
        //*********修改*********//
        //存储form相关信息
        storeFormData(subUrl);

        // 将数据填充到 typeSet, ResultMap, firstFieldMap
        initData(jsonArray);
        ...
        for (Map<String, Map<String, String>> stringMapMap : subList){
            ...
            for (String typeToSave : typeSet) {
                ...
                if (typeMap != null) {
                    saveDocByType(typeMap, typeToSave, newDocument);
                }
            }
    ...
    public void saveDocByType(Map<String, String> typeMap, String typeToSave, Document newDocument) throws Exception{
        // 获取集合
        ...
        for (Map<String, Map<String, String>> stringMapMap : subList) {
            for(Map.Entry<ObjectId,String> entry : docIdMapList.get(index).entrySet()) {
                ...
                for(Map.Entry<ObjectId,String> entry_ : docIdMapList.get(index).entrySet()) {
                    // entry_ 存放插入对象信息
                    ObjectId idToBuild = entry_.getKey();
                    String typeToBulid = entry_.getValue();
                    // 两个文档间建立单向关系
                    makeAConnection(idToBuild, typeToBulid, id, type);
                }
            }
            index = index + 1;
        }
    ...
    //两文档间建立单向关系
    //将 id 插入 ObjectId 为 _id 的文档中
    public void makeAConnection(ObjectId _id, String _type, ObjectId id, String type ) throws Exception{
        //不和自己建立关系
        if(id.equals(_id))
            return;

        MongoCollection<Document> tempCollection = testDb.getCollection(_type);
        // 建立查询用的 Document，使用 _id 查询
